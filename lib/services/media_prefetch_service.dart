...
class MediaPrefetchService {
  ...
  void prefetchReelsVideos(
    ...
    // Phase 4.1: Skip prefetch if video is paused
    if (!isVideoPlaying) {
      debugPrint('MediaPrefetchService: Skipping prefetch - video is paused');
      return;
    }

    // CRITICAL FIX: Skip prefetch on fast scrolling (velocity > 1.5 pages/second)
    // Reduced threshold to be more aggressive about cancellation
    if (scrollVelocity > 1.5) {
      debugPrint('MediaPrefetchService: Skipping prefetch - fast scrolling (velocity: $scrollVelocity)');
      // Cancel any active prefetch operations immediately
      _cancelActivePrefetchOperations();
      return;
    }

    // Determine which reel IDs to keep (current + next few)
    final keepReelIds = <String>[];
    for (int i = 0; i <= 2 && (currentIndex + i) < reels.length; i++) {
      keepReelIds.add(reels[currentIndex + i].reelId);
    }

    // Clean up distant controllers (Phase 3 - improved cleanup)
    clearDistantControllers(keepReelIds);

    // Phase 4.1: Cancel prefetch operations for reels that are no longer next
    _cancelPrefetchForDistantReels(keepReelIds);

    // Clear old queue
    _videoQueue.clear();

    // Phase 1.3: Prefetch only next 1 video (reduced from 2 to minimize memory)
    for (int i = 1; i <= 1 && (currentIndex + i) < reels.length; i++) {
      final reel = reels[currentIndex + i];
      final reelId = reel.reelId;

      // Only prefetch if not already prefetched or in queue
      if (!_prefetchedControllers.containsKey(reelId) &&
          !_videoQueue.contains(reelId)) {
        _videoQueue.add(reelId);
        // Clear cancellation flag for this reel
        _prefetchCancelled[reelId] = false;
        
        // Add delay between prefetch operations to prevent codec exhaustion
        final prefetchFuture = Future.delayed(Duration(milliseconds: 300 * i), () {
          if (!_isDisposed && !(_prefetchCancelled[reelId] ?? false)) {
            _prefetchVideo(reel, reelId); // Pass entire reel model for ABR
          }
        });
        
        // Phase 4.1: Track active prefetch operation
  ...
  /// The core prefetching logic with retry support.
  ///
  /// Phase 4.1: Enhanced with cancellation support
  /// This method:
  ...
  Future<void> _prefetchVideo(ReelModel reel, String reelId,
    ...
    try {
      ...
      final String videoUrlToPrefetch =
          reel.getVideoUrlForQuality(currentQuality);
      ...
      try {
        ...
        _videoQueue.remove(reelId);
        _activePrefetchOperations.remove(reelId);
        _prefetchCancelled.remove(reelId);
        return;
      }
      ...
      debugPrint('MediaPrefetchService: Error prefetching $reelId: $e');
      // Remove from queue and active operations on error
      _videoQueue.remove(reelId);
      _activePrefetchOperations.remove(reelId);
      _prefetchCancelled.remove(reelId);
    } finally {
      _videoQueue.remove(reelId); // Remove from queue when done
    }
  }
