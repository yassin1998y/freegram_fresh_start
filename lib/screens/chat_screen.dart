...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _onTyping() {
    ...
    _typingTimer =
        Timer(const Duration(milliseconds: 1500), () => _updateTypingStatus(false));
  }
  ...
  Future<String?> _uploadToCloudinary(File file,
    ...
    final url =
    Uri.parse('https://api.cloudinary.com/v1_1/dq0mb16fk/$resourceType/upload');
    final request = http.MultipartRequest('POST', url)
      ..fields['upload_preset'] = 'Prototype';
    final multipartFile = http.MultipartFile.fromBytes('file', await file.readAsBytes(),
  ...
  void _startReply(Message message) {
    setState(() {
      ...
      _replyingToSender = message.senderId == FirebaseAuth.instance.currentUser!.uid
          ? 'You'
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, chatSnapshot) {
        ...
        final bool isSender = isContactRequest && currentUser.uid == initiatorId;
...
class _SenderInfoBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        "You can send up to 2 messages. Once they reply or accept your request, you can chat freely.",
    ...
    if (isMe) {
      switch (message.status) {
        case MessageStatus.sending:
          statusWidget =
          const Text('sending...', style: TextStyle(fontSize: 10, color: Colors.grey));
          break;
          ...
          children: [
                ...
                if (message.reactions.isNotEmpty)
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Text(
                          message.reactions.values.toSet().toList().join(' '),
            ...
            Padding(
              padding: EdgeInsets.only(
                  top: message.reactions.isNotEmpty ? 12.0 : 4.0,
                  left: isMe ? 0 : 8.0,
                  right: isMe ? 8.0 : 0),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  if (message.timestamp != null)
                    Text(timeago.format(message.timestamp!.toDate(), locale: 'en_short'),
                        style:
