...
class _CreateReelScreenState extends State<CreateReelScreen> {
  ...
  Future<Map<String, String>?> _uploadToCloudinary(File video) async {
    ...
    if (response.statusCode == 200) {
      ...
      final videoUrl = jsonMap['secure_url'] as String?;
      if (videoUrl == null) return null;

      final thumbnailUrl = videoUrl.replaceAll('/upload/', '/upload/w_400,h_600,c_fill/')
          .replaceAll('.mp4', '.jpg');
  ...
  Future<void> _createReel() async {
    ...
    try {
      // FIX: Standardize frame rate to 30 for smaller file size.
      final mediaInfo = await VideoCompress.compressVideo(
        _editedVideoFile!.path,
        quality: VideoQuality.MediumQuality,
        deleteOrigin: false,
        frameRate: 30, // Ensures videos aren't unnecessarily large
      );
      ...
      await locator<PostRepository>().createPost(
