...
// Replace the entire _exportVideo method with this one.
...
Future<void> _exportVideo() async {
  ...
  try {
    ...
    // --- Step 1: Extract Font from Assets ---
    // FFmpeg needs a direct file path to a font to draw text reliably.
    final fontAsset = await rootBundle.load('assets/fonts/OpenSans-Regular.ttf');
    final fontFile = File('${tempDir.path}/OpenSans-Regular.ttf');
    await fontFile.writeAsBytes(fontAsset.buffer.asUint8List(
      fontAsset.offsetInBytes,
      fontAsset.lengthInBytes,
    ));

    // --- Step 2: Create temporary files for each text overlay ---
    // This is the safest way to handle special characters in text.
    List<File> textFiles = [];
    String drawTextFilter = "";
    for (var i = 0; i < _textOverlays.length; i++) {
      final overlay = _textOverlays[i];
      final textFile = File('${tempDir.path}/text_overlay_$i.txt');
      // Sanitize text for file writing, just in case.
      await textFile.writeAsString(overlay.text.replaceAll("'", "'\\''"));
      textFiles.add(textFile);

      // Use 'fontfile' and 'textfile' to prevent crashes.
      drawTextFilter +=
          "drawtext=fontfile='${fontFile.path}':textfile='${textFile.path}':x=${overlay.position.dx}:y=${overlay.position.dy}:fontsize=${overlay.size}:fontcolor=${_colorToHex(overlay.color)},";
    }

    // --- Step 3: Build the rest of the FFmpeg command ---
    String musicInput = "";
    ...
    if (_selectedMusicAsset != null) {
      ...
      audioFilterGraph = ";[0:a]volume=0.5[a0];[1:a]volume=1.0[a1];[a0][a1]amix=inputs=2:duration=first[a]";
      mapAudio = "-map [a]";
    }

    final colorFilter = _selectedFilterMatrix != null
        ? "colorchannelmixer=matrix=${_selectedFilterMatrix!.join('\\:')},"
        : "";

    // --- Step 4: Construct and Execute the Command ---
    final command =
        '''-i '${widget.file.path}' $musicInput -ss ${_controller.startTrim.inMilliseconds / 1000} -to ${_controller.endTrim.inMilliseconds / 1000} -filter_complex "[0:v]format=yuv420p,${colorFilter}${drawTextFilter}scale=720:-2[v]$audioFilterGraph" -map "[v]" $mapAudio -c:v libx264 -preset ultrafast '$outputPath' ''';
