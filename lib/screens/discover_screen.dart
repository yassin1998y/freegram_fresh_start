...
class _ExploreTabState extends State<ExploreTab> {
  ...
  Future<void> _getUsers({bool isRefresh = false}) async {
    ...
    try {
      final querySnapshot = await locator<UserRepository>().getPaginatedUsers(
  ...
  Future<void> _getNearbyUsers() async {
    ...
    for (var key in nearbyBox.keys) {
      ...
      if (profileData != null) {
        // FIX: Use the new UserModel.fromMap constructor
        nearbyUsers.add(
            UserModel.fromMap(key, Map<String, dynamic>.from(profileData)));
      } else {
        try {
          final userModel = await locator<UserRepository>().getUser(key);
...
class _ForYouTabState extends State<ForYouTab> {
  ...
  Future<void> _fetchRecommendedUsers() async {
    ...
    try {
      final user = await locator<UserRepository>().getUser(currentUser.uid);
      ...
      if (!mounted) return;
      final userDocs = await locator<UserRepository>()
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () async {
                  final currentUser = FirebaseAuth.instance.currentUser!;
                  final user =
                  await locator<UserRepository>().getUser(currentUser.uid);
                  if (mounted) {
...
class CompactUserCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (user.country.isNotEmpty) {
      try {
        final country =
        CountryService().getAll().firstWhere((c) => c.name == user.country);
      ...
      onTap: () {
        showModalBottomSheet(
          ...
          builder: (_) => BlocProvider(
            ...
            child: UserInfoPopup(userId: user.id),
          ),
        );
      },
      child: Card(
    ...
    return Shimmer.fromColors(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (context, index) => Card(
            clipBehavior: Clip.antiAlias, child: Container(color: Colors.white)),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return DraggableScrollableSheet(
      ...
      builder: (_, scrollController) => StreamBuilder<UserModel>(
        stream: locator<UserRepository>().getUserStream(widget.userId),
