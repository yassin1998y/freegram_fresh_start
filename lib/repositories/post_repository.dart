...
class PostRepository {
  ...
  // FIX: Declared the final fields for the repository dependencies.
  ...
  // FIX: Initialized the declared fields from the constructor parameters.
  ...
  // FIX: Replaced the invalid 'whereIn' + 'orderBy' query with a scalable
  // client-side merge strategy.
  Future<List<DocumentSnapshot>> getFeedPosts(String currentUserId,
    ...
    for (String id in friendIds) {
      Query query = _db
          ...
          .limit(5); // Fetch 5 most recent posts per friend
    ...
    // Sort all the fetched posts by timestamp on the client-side
    allPosts.sort((a, b) {
    ...
    // We no longer support pagination with this model, so we return the top results.
